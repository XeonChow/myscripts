...
print_temp(){
	...
	echo "$(head -c 2 /sys/class/thermal/thermal_zone0/temp)糖"
}
...
get_battery_combined_percent() {
    ...
    percent=$(acpi | cut -d, -f2 | cut -d% -f1 | cut -d" " -f2)  #$(expr $total_charge / $battery_number);
	if $(acpi -b | grep --quiet Discharging)
	then
        if test $percent -gt 95
	    then
            echo "$percent%"
        elif test $percent -gt 90
        then
            echo "$percent%"
        elif test $percent -gt 80
        then
            echo "$percent%"
        elif test $percent -gt 70
        then
            echo "$percent%"
        elif test $percent -gt 60
        then
            echo "$percent%"
        elif test $percent -gt 50
        then
            echo "$percent%"
        elif test $percent -gt 40
        then
            echo "$percent%"
        elif test $percent -gt 30
        then
            echo "$percent%"
        elif test $percent -gt 20
        then
            echo "$percent%"
        elif test $percent -gt 10
        then
            echo "$percent%"
        else
            echo "$percent%"
...
xsetroot -name " $(print_mem)M $vel_recv $vel_trans $(print_temp) $(print_monitor_brightness) $(print_volume) $(print_bat)$(show_record) $(print_date) "
